...
namespace EdiProcessingUnit.Edo.Models
{
    public class UniversalTransferDocumentDetail
    {
        ...
        private Enums.HonestMarkGoodType _honestMarkGood = Enums.HonestMarkGoodType.None;
        ...
        public UniversalTransferDocumentDetail Init(AbtDbContext abtContext, RefEdoGoodChannel edoGoodChannel = null)
        {
            ...
            if(_good != null)
            {
                var honestMarkGood = abtContext?.RefItems?.FirstOrDefault(r => r.IdName == 30071 && r.IdGood == _good.Id)?.Quantity ?? 0;
